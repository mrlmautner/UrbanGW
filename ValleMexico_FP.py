...
VK_PAR = [100,100,10,0.1,0.01]
# COL2 : Zone specific storage
SS_PAR = [6.56E-02,8.077E-04,1.430E-02,2.313E-07,1.64E-05]
# COL3 : Zone hydraulic conductivity
HK_PAR = [4.32E-03,9.270E+00,7.615E-03,8.284E+00,3.315E-02] # m/d
...
R_mult = {0:[0.01,0.267,0.5],1:RCH_PAR,2:RCH_PAR,3:RCH_PAR}
...
for i, LUset in enumerate(['1990','2000','2010']):
    RCH_DICT = mod.addRecharge(LU_arrays=LU[LUset]['ARRAY'],PRECIP=Precip_Dict,S_YR=start[i],E_YR=end[i],RCH_Dict=RCH_DICT,RCH_mult=R_mult[i])
...
WEL_DICT, WEL_INFO = mod.addNewWells(PUMP_array,LYR=1,dateType='per',mun=MUN)
PUMP_array = np.loadtxt(r'data_output\wells\PUMP_S.csv',
                                              delimiter=',', skiprows=1, usecols=[1,2,7,8,11]) # pumping in m3 per day
WEL_DICT, WEL_INFO = mod.addNewWells(PUMP_array,LYR=1,WEL_Dict=WEL_DICT,INFO_Dict=WEL_INFO,dateType='per',mun=MUN)
...
for i, wellset in enumerate(['PUMP_RC_Q','PUMP_RC_Q','PUMP_RC_Q']):
    ...
    WEL_DICT, WEL_INFO = mod.addNewWells(New_WEL=PUMP_array,LYR=1,WEL_Dict=WEL_DICT,INFO_Dict=WEL_INFO,WEL_mult=PUMP_PARAM[i],S_YR=start[i],E_YR=end[i],mun=MUN)
...
for i, leakset in enumerate(['1990','2000','2010']):
    LEAK_MUN = np.loadtxt('data_output\leak\LEAK_TOT_MUN.csv',delimiter=',',skiprows=1,usecols=[i+1]) # Total leak per municipality m3/d
    ...
    WEL_DICT, WEL_INFO = mod.addNewWells(LEAK_arrays[leakset],LYR=1,WEL_Dict=WEL_DICT,INFO_Dict=WEL_INFO,WEL_mult=LEAK_PAR[i],coordType='rc0',mun=MUN,wellType=1)
