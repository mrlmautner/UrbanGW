...
tot_samples = 100
soswrlim = 2000000
safolder = '20200403_100'
numSets = 10
currentSet = 0
sarun = int((tot_samples/numSets)*currentSet)

try:
    firstSample = Path.cwd().joinpath('model_files').joinpath('output').joinpath('sa').joinpath('err').joinpath(safolder).joinpath('{:05d}'.format(sarun)+'.csv')
    f = open(firstSample)
    f.close()
    print('This set has already been completed')
except:
    ...
    def master(comm):
        ...
        while True:
            ...
            if sarun == (tot_samples/numSets)*(currentSet+1):
                break
