...
if my_rank==0:
    ...
    np.savetxt(str(outputpath.joinpath('analysis').joinpath(folder + ('-norm' if kmeans_norm else '') + ('-t' if kmeans_time else '') + '-' + str(n_clusters) + '-cluster.csv')), group_label, delimiter=',')
...
if my_rank==0:
    ...
    np.savetxt(str(outputpath.joinpath('analysis').joinpath(folder + ('-norm' if kmeans_norm else '') + ('-t' if kmeans_time else '') + '-' + str(n_clusters) + '-samples_evald.csv')), err_obj_array[:,0], delimiter=',') # Save ids of all samples to be tested
